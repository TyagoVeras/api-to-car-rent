...
class UpdateAvatarUseCase{
    ...
    async execute({ userId, avatarPath}: IRequest): Promise<Users>{
        ...
        const userUpdated = await this.userRepository.update(userId, { name: user.name, driverLicense: user.driver_license, email: user.email, password: user.password, avatar: avatarPath });
        return userUpdated;
    }
}
